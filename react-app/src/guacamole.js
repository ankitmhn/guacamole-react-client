(function () {
   ...
   *     false otherwise.
   */
  Guacamole.RawAudioPlayer.isSupportedType = function isSupportedType(
    mimetype
  ) {
  ...
  Guacamole.Display = function () {
    ...
    display.style.transformOrigin = display.style.webkitTransformOrigin = display.style.MozTransformOrigin = display.style.OTransformOrigin = display.style.msTransformOrigin =
      "0 0";
    ...
    this.scale = function (scale) {
      display.style.transform = display.style.WebkitTransform = display.style.MozTransform = display.style.OTransform = display.style.msTransform =
        "scale(" + scale + "," + scale + ")";
  ...
  Guacamole.Display.VisibleLayer = function (width, height) {
    ...
    this.translate = function (x, y) {
      ...
      // Set layer transform
      div.style.transform = div.style.WebkitTransform = div.style.MozTransform = div.style.OTransform = div.style.msTransform =
        translate + " " + matrix;
    };
  ...
  Guacamole.Mouse = function (element) {
    ...
    element.addEventListener(
      ...
      function (e) {
        ...
        guac_mouse.currentState.fromClientPosition(
          element,
          e.clientX,
          e.clientY
        );
  ...
  Guacamole.OnScreenKeyboard = function (layout) {
    ...
    keyboard.onselectstart = keyboard.onmousemove = keyboard.onmouseup = keyboard.onmousedown = function handleMouseEvents(
      e
    ) {
      // If ignoring events, decrement counter
  ...
  Guacamole.WebSocketTunnel = function (tunnelURL) {
    ...
    this.connect = function (data) {
      ...
      socket.onerror = function (event) {
        close_tunnel(
          new Guacamole.Status(Guacamole.Status.Code.SERVER_ERROR, event.data)
        );
      };
    ...
    this.connect = function connect(data) {
      ...
      xhr.onerror = function httpError() {
        ...
        if (tunnel.onerror)
          tunnel.onerror(
            new Guacamole.Status(
              getGuacamoleStatusCode(xhr.status),
              xhr.statusText
            )
          );
   ...
   */
  Guacamole.SocketIOTunnel = function (
    tunnelURL,
    connectionOptions,
    eventChannel
  ) {
    ...
    this.connect = function (data) {
      ...
      // FOLLOWING LINE GENERATES BUILD ERROR - SOCKET TUNNEL DOES NOT WORK!
  ...
  if (module && module.exports) module.exports = Guacamole;
  else window.Guacamole = Guacamole;
})();
